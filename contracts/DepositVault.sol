...
contract DepositVault is ManageableVault, IDepositVault {
    ...
    error DepositVault_InvalidFirewall(address firewall);
     ...
     */
    uint256 public minZTokenAmountForFirstDeposit;
    ...
    /**
     * @notice max supply cap value in zToken
     * @dev if after the deposit, zToken.totalSupply() > maxSupplyCap,
     ...
     * @param _minAmount basic min amount for operations in zToken
     * @param _minZTokenAmountForFirstDeposit min amount for first deposit in zToken
     * @param _maxSupplyCap max supply cap for zToken
    ...
    function initialize(
        ...
        uint256 _minAmount,
        uint256 _minZTokenAmountForFirstDeposit,
        uint256 _maxSupplyCap,
        address _firewall
    ) public {
        if (_firewall == address(0))
            revert DepositVault_InvalidFirewall(_firewall);
        initializeV1(
            _ac,
            _zTokenInitParams,
            ...
            _variationTolerance,
            _minAmount,
            _minZTokenAmountForFirstDeposit
        );
    ...
    function initializeV1(
        ...
        minZTokenAmountForFirstDeposit = _minZTokenAmountForFirstDeposit;
    }
     ...
     * @inheritdoc IDepositVault
     */
    function setMinZTokenAmountForFirstDeposit(
        uint256 newValue
    ) external onlyVaultAdmin {
        minZTokenAmountForFirstDeposit = newValue;

        emit SetMinZTokenAmountForFirstDeposit(msg.sender, newValue);
    }
     ...
     * @param minReceiveAmount min amount of zToken to receive (decimals 18)
    ...
    function _depositInstant(
        ...
        zToken.mint(recipient, result.mintAmount);
     ...
     * Mints zTokens to user
     * @param requestId request id
     * @param newOutRate zToken rate
     */
    function _approveRequest(
        ...
        uint256 amountZToken = (request.usdAmountWithoutFees * (10 ** 18)) /
            newOutRate;

        if (!_validateMaxSupplyCap(amountZToken, revertAboveSupplyCap)) {
            return false;
        }

        zToken.mint(request.sender, amountZToken);

        totalMinted[request.sender] += amountZToken;
    ...
    function _calcAndValidateDeposit(
        ...
        (uint256 zTokenAmount, uint256 zTokenRate) = _convertUsdToZToken(
            result.tokenAmountInUsd - feeInUsd
        );
        result.mintAmount = zTokenAmount;
        result.tokenOutRate = zTokenRate;
     ...
     * @param user user address
     * @param amountZTokenWithoutFee amount of zToken without fee (decimals 18)
     */
    function _validateMinAmount(
        address user,
        uint256 amountZTokenWithoutFee
    ) internal view {
        require(amountZTokenWithoutFee >= minAmount, "DV: zToken amount < min");

        if (totalMinted[user] != 0) return;

        require(
            amountZTokenWithoutFee >= minZTokenAmountForFirstDeposit,
            "DV: mint amount < min"
        );
    }

    /**
     * @dev validates that zToken.totalSupply() <= maxSupplyCap
     *
     ...
     * @dev validates that zToken.totalSupply() <= maxSupplyCap
     *
     * @param mintAmount amount of zToken to mint
     * @param revertOnError if true, will revert if supply is exceeded
     * if false, will return false if supply is exceeded without reverting
     *
     * @return true if supply is valid, false otherwise
    ...
    function _validateMaxSupplyCap(
        ...
        bool isExceeded = zToken.totalSupply() + mintAmount > maxSupplyCap;
    ...
    function _convertTokenToUsd(
        address tokenIn,
        uint256 amount
    ) internal view virtual returns (uint256 amountInUsd, uint256 rate) {
    ...
    /**
     * @dev calculates zToken amount from USD amount
     * @param amountUsd amount of USD (decimals 18)
     *
     * @return amountZToken converted USD to zToken
